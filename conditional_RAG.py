...
def build_retriver(pdf_path : str):
    ...
    vectorstore = FAISS.from_documents(chunks,embeddings) #FAISS works in internal system, no folder created
...
def response_node(state: State) -> dict:
    ...
    context = state["retrieved_context"] #academic, fees or general 
    ...
    response = llm.invoke(prompt)
    return {"messages": [("ai", response.content.strip())]} #inlanggraph we can store messages as a history
